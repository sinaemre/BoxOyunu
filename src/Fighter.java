...
public class Fighter {
    ...
    Fighter(String nickName, double weight) {
    ...
    private void calcDamage() { // kilosu az olan yüksek hasar vursun!
    ...
    private void calcDodge() { // kilosu fazla olanın bloklama oranı yüksek olsun!
        if (this.weight >= 40 && this.weight < 50) {
    ...
    private int getRandomNumber(int min, int max) { // bloklama ve hasar oranı için belli aralıktan sayı üretme methodu
        Random random = new Random();
        return random.nextInt(max - min) + min;
    }

    protected void showFightersInfo() {
